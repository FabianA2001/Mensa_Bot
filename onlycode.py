...
async def setNumTasks(ctx, num):
    ...
    numTasks = int(num)
    embed = discord.Embed(
        description=" -- " + str(num) + " tasks will be send instead now."
    )
    await ctx.send(embed=embed)
...
async def addTask(ctx, weight, newTask):
    ...
    if not weight.isnumeric:
        embed = discord.Embed(
            description="Error: first value must be an integer (qutations are not allowed)"
        )
        await ctx.send(embed=embed)
        return
...
async def removeTask(ctx, removeTask):
    ...
    if type(removeTask) != str:
        embed = discord.Embed(
            description='Error: second value must be an string (ex. "Read book").'
        )
        await ctx.send(embed=embed)
        return
    global tasks
    for weight in tasks:
        if removeTask in tasks[weight]:
            ...
            embed = discord.Embed(
                description="-- Removed item from " + str(weight) + " weight class."
            )
            await ctx.send(embed=embed)
            return
...
async def setMessageTime(ctx, newTime):
    if not ":" in newTime:
        embed = discord.Embed(
            description="Error: time must be in hour:minutes and 24 hour format (ex. 20:35)."
        )
        await ctx.send(embed=embed)
        return
    ...
    if newMinutes > 59 or newMinutes < 0:
        embed = discord.Embed(
            description="Error: invalid minutes. Must be from 0 to 59"
        )
        await ctx.send(embed=embed)
        return
    global hour, minutes
...
async def blockDays(ctx, *days):
    ...
    for day in days:
        if not day.lower() in possibleDays:
            embed = discord.Embed(
                description="Error: " + str(day) + " is not a valid day. Nothing added."
            )
            await ctx.send(embed=embed)
            return
        if day.lower() in blacklistedDays:
            embed = discord.Embed(
                description=str(day) + " is already black listed. It will be ignored"
            )
            await ctx.send(embed=embed)
    ...
    message = (
        " -- Days added to blacklist"
        if len(days) > 1
        else " -- Day added to blacklist."
    )
...
async def unblockDays(ctx, *days):
    ...
    for day in days:
        ...
        if not day.lower() in blacklistedDays:
            embed = discord.Embed(
                description=str(day) + " is not black listed. It will be ignored"
            )
            await ctx.send(embed=embed)
            days.remove(day.lower())
    ...
    message = (
        " -- Days removed from blacklist"
        if len(days) > 1
        else " -- Day removed from blacklist."
    )
    embed = discord.Embed(description=message)
    await ctx.send(embed=embed)
...
async def dmTasks():
    ...
    itemCount = 1
    embed = discord.Embed(
        title="Tasks for " + todaysDate, description=todaysMessage, color=1752220
    )
